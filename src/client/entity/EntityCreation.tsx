...
const EntityCreation: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    // Send API request
    try {
      const response = await fetch("https://backend-5n7t.onrender.com/api/entity/create", {
        method: "POST",
  ...
  //     const levelString = `Level ${levelMinusOne}`;
  //      console.log("Fetching parent entities for:", levelString);

  //     const timestamp = new Date().getTime();
  //     axios
  //       .get(
  //         `https://backend-5n7t.onrender.com/api/entity/findParentAtLevel/${levelString}?t=${timestamp}`
  //       )
  //       .then((res) => {
  //         const names = res.data
  //           .map((item: any) => item.parentname)
  //           .filter(Boolean);

  //         // Cast the unique set as string[]
  //         const uniqueNames = Array.from(new Set(names)) as string[];

  //         setParentOptions(uniqueNames);
  //          console.log("Fetched unique parents:", uniqueNames);
  //       })
  //       .catch((err) => {
  ...
  //     const timestamp = new Date().getTime(); // cache buster to avoid cached API response

  //     axios
  //       .get(`https://backend-5n7t.onrender.com/api/entity/findParentAtLevel/${encodeURIComponent(levelString)}?t=${timestamp}`)
  //       .then((res) => {
  //         if (Array.isArray(res.data)) {
  //           const names = res.data
  //             .map((item: any) => item.entity_name)
  //             .filter((name: string) => !!name); // remove falsy
  ...
  //      console.log("Fetching parent entities for level:", parentLevel);
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://backend-5n7t.onrender.com/api/entity/findParentAtLevel/${parentLevel}`)
      .then((res) => {
        if (Array.isArray(res.data)) {
          const uniqueNames = [
