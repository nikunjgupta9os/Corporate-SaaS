...
import { useNotification } from "../../Notification/Notification.tsx";
  ...
  interface TableProps<ExposureBucketing extends WithId> {
    ...
    onUpdate?: (rowId: string, changes: Partial<ExposureBucketing>) => Promise<boolean>;
    className?: string;
  ...
  return changed;
    ...
    // const getChangedFields = (original: ExposureBucketing, edited: Partial<ExposureBucketing>): Partial<ExposureBucketing> => {
    //   const changes: Partial<ExposureBucketing> = {};
    //   for (const key in edited) {
    //     if (edited[key] !== original[key]) {
    ...
    //     const referenceNo = row.original.id;
    //     try {
    ...
    //           remarks: ("remarks" in changedFields) ? changedFields["remarks"] : undefined,
    //           inco: ("inco" in changedFields) ? changedFields["inco"] : undefined,
    //           advance: ("advance" in changedFields) ? changedFields["advance"] : undefined,
    //           month_1: ("month_1" in changedFields) ? changedFields["month_1"] : undefined,
    //           month_2: ("month_2" in changedFields) ? changedFields["month_2"] : undefined,
    //           month_3: ("month_3" in changedFields) ? changedFields["month_3"] : undefined,
    //           month_4_6: ("month_4_6" in changedFields) ? changedFields["month_4_6"] : undefined,
    //           month_6plus: ("month_6plus" in changedFields) ? changedFields["month_6plus"] : undefined,
    //         }),
    //       });
    ...
    const handleEditToggle = async () => {
      if (isEditing) {
        ...
        let success = false;
        if (onUpdate) {
          success = await onUpdate(row.original.id, changedFields);
        } else {
          // fallback: try direct API call (should not happen in production)
          try {
...
//         setIsEditing(false);
//       } else {
//         console.error("Failed to update exposure:", result.error || result);
//       }
//     } catch (error) {
//       console.error("Error during update:", error);
//     }
...
// };
    ...
    const renderField = (key: string) => {
      ...
      const value = isEditing ? editValues[key as keyof ExposureBucketing] : originalValue;
      ...
      if (key === "remaining_percentage") {
        ...
        const getNum = (field: string): number => {
          const v = isEditing ? editValues[field as keyof ExposureBucketing] : row.original[field as keyof ExposureBucketing];
          return v === undefined || v === null || v === '' ? 0 : Number(v);
        };
        const po_amount = getNum('po_amount');
        ...
        const totalPaid = advance + month_1 + month_2 + month_3 + month_4_6 + month_6plus;
        const remaining = po_amount - totalPaid;
        const percentage = po_amount ? (remaining / po_amount) * 100 : 0;
        ...
        return (
          <div key={key} className="flex flex-col space-y-1">
            ...
            <span className={`text-sm font-semibold ${color}`}>{percentage.toFixed(2)}%</span>
          </div>
        );
      }
            ...
            key === "inco" ? (
              <>
                <select
                  ...
                  onChange={(e) => handleChange(key as keyof ExposureBucketing, e.target.value)}
                >
              ...
              <>
                <select
                  ...
                  value={String(editValues[key as keyof ExposureBucketing])}
                  onChange={(e) =>
                    handleChange(key as keyof ExposureBucketing, e.target.value === "true")
                  }
                >
          ...
          <div className="bg-secondary-color-lt rounded-lg p-4 shadow-md border border-border">
            ...
            {config.sections.map((section) => (
              <div key={section.title} className="mb-6">
                <div className="font-semibold mb-2 text-primary-lt">{section.title}</div>
