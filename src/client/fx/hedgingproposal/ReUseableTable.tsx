...
function ExpandedRow<T extends EditableRowData>({
  ...
  const renderField = (key: string) => {
    ...
    let value: any = isEditing ? editValues[key as keyof T] : row.original[key as keyof T];
    ...
    return (
      <div key={key} className="flex flex-col space-y-1">
        ...
        {isEditing && isEditable ? (
          ...
          <span className="font-medium text-primary-lt">{String(value ?? "—")}</span>
        )}
      </div>
    );
  };
...
function NyneOSTable<T extends EditableRowData>({
  ...
  const table = useReactTable({
    ...
    meta: {
  updateData: (rowIndex: number, updatedRow: T) => {
    if (!onUpdate) return;
    const rowId = updatedRow.id;
    const changes: Partial<T> = {};

    for (const key in updatedRow) {
      if (data[rowIndex][key] !== updatedRow[key]) {
        changes[key] = updatedRow[key];
      }
    }

    if (Object.keys(changes).length > 0) {
      onUpdate(rowId, changes).then(success => {
        if (success) {
          Object.assign(data[rowIndex], changes); // force update of data reference
        }
      });
    }
  }
