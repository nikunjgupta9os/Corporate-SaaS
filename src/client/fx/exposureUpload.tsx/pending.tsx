...
//       alert(response.data.message || "Exposures marked for delete approval");
//       // Optionally refresh table data
//     } else {
//       alert(response.data.message || "Failed to delete exposures");
//     }
//   } catch (error) {
//      console.error("Error deleting exposures:", error);
//     alert("An error occurred while deleting exposures");
//   }
// };
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await axios.post(
        "https://backend-5n7t.onrender.com/api/exposureUpload/bulkApprove",
        {
        ...
        if (setData) {
          const updatedData = data.map((item) => {
            ...
            if (selectedUserIds.includes(item.id)) {
              return { ...item, status: "approved" }; // Update status or your relevant field
            }
            return item;
          });
          setData(updatedData);
    ...
    // Uncomment and implement the handleReject function if needed
  ...
  try {
    const response = await axios.post(
      "https://backend-5n7t.onrender.com/api/exposureUpload/bulkReject",
      {
        exposureIds: selectedUserIds,
...
//     try {
//       // Example API call - adjust to your actual API endpoint
//       const response = await axios.post(
//         "https://backend-5n7t.onrender.com/api/exposure/bulkReject",
//         {
...
//       if (response.data.success) {
//         // Update the local data state if needed
//         if (setData) {
//           const updatedData = data.map((item) => {
//             if (selectedRows.some((row) => row.original.id === item.id)) {
//               return { ...item, status: "rejected" }; // adjust the field as needed
//             }
//             return item;
//           });
//           setData(updatedData);
...
//         alert(`${selectedRows.length} item(s) rejected successfully`);
//       } else {
//         throw new Error(response.data.message || "Rejection failed");
//       }
//     } catch (error) {
